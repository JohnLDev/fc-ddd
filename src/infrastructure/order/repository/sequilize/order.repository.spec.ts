...
describe('Order repository test', () => {
  ...
  it('should find an order', async () => {
    ...
    expect(orderFound.id).toStrictEqual(orderModel.id);
  });
  ...
  it('should update an order', async () => {
    ...
    await orderRepository.create(order);
    order.items.push(
      new OrderItem('2', product.name, product.price, product.id, 1)
    );
    await orderRepository.update(order);

    const newOrder = await orderRepository.find(order.id);
    expect(newOrder.total()).toBe(30);
  ...
  it('should be able to find all orders', async () => {
    ...
    expect(orders[1].total()).toBe(100);
    expect([order1, order2]).toStrictEqual(orders);
  });
});
